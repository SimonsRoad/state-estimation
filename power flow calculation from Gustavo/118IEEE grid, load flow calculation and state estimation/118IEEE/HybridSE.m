...
% I Phasor PMU
%=========================
K=load('I phasor PMU Measured Rectangular.txt');
nI=length(K(:,1));
imc=K(:,4);
sigmai=K(:,5:6);
...
i2=1;
for i=1:nI/2
    Ra(i1,i1)=sigmai(i2,1); 
    Ra(i1,i1+1)=sigmai(i2,2);
    Ra(i1+1,i1)=sigmai(i2+1,1); 
    Ra(i1+1,i1+1)=sigmai(i2+1,2);
    i1=i1+2;
    i2=i2+2;
end
...
evalfuncHSE(States,B,Bus,Cp,Cq,Dp,Dq,E,F,K,Shunt)

% Jacobian Calculation
%========================
jacobianHES(States,B,Bus,Cp,Cq,Dp,Dq,E,F,K,Shunt)
% 
 if slackyes==1
...
% =========================================
sR=0;
for i=1:length(diag(R))
    sR=sR+1/R(i,i);
end

alpha=length(diag(R))/sR;
...
Ztrue=[Piog;Qiog;Ptog;Qtog;uog;vog;cog;Iog];
evalfuncHSE(States,B,Bus,Cp,Cq,Dp,Dq,E,F,K,Shunt)
